...
public class MailUtil {
	...
	public  static boolean sendEmail(SendEntryMailBo sendEntryMailBo){
	...
	private static boolean publicsendEmail(SendEntryMailBo sendEntryMailBo){
		...
		try {
			...
			Message message =!sendEntryMailBo.getIsGroup() ? createEmail(session,userName,targetAddress,mimeMessageDTO)
					:createEmailByGroupAndFile(session,userName,
					targetAddress,mimeMessageDTO,filepath==null?null:filepath);
	...
	private static boolean publicsendEmail(String userName,String password,String targetAddress,
		...
		Properties props = makeMailProperties(userName, null);
		String hostname=SMTPUtil.getSMTPAddress(userName, null);
		Session session = Session.getInstance(props, new PopupAuthenticator(userName, password));
		session.setDebug(true);
		try {
			Transport ts = session.getTransport();
